// credits: afl_ext 2017-2024
// MIT License
// adapted for Ghostty from https://www.shadertoy.com/view/Ms2SD1
// Optimized lite version with reduced iterations
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec3 ray = getRay(flippedCoord);

  if(ray.y >= 0.0) {
    vec3 C = getAtmosphere(ray) + getSun(ray);
    vec4 oceanColor = vec4(aces_tonemap(C * 2.0) * 0.02, 0.02);

    vec2 uv = fragCoord/iResolution.xy;
    vec4 terminalColor = texture(iChannel0, uv);
    vec3 blendedColor = terminalColor.rgb + oceanColor.rgb;
  ...
  vec4 oceanColor = vec4(aces_tonemap(C * 2.0) * 0.03, 0.03);
