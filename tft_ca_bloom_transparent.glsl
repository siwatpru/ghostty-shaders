// tft_ca_bloom_transparent.glsl  —  TFT scanlines + mild CA + light noise (no warp, no glitch) with transparent background support
...
float bloom_general   = 0.15;  // overall bloom intensity (all pixels)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec4 texR = texture(iChannel0, uv + eps);
    vec4 texG = texture(iChannel0, uv       );
    vec4 texB = texture(iChannel0, uv - eps );
    ...
    // Preserve original alpha (average of the three samples)
    float alpha = (texR.a + texG.a + texB.a) / 3.0;

    // --- General bloom (overall glow) ---
    if (bloom_general > 0.0 && alpha > 0.0) {
        vec3 bloom = vec3(0.0);
    ...
    if (bloom_selective > 0.0 && alpha > 0.0) {
